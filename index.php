	...
	public function holePosts($posts) {
		...
		while(isset($posts['posts'][$i])) {
			...
			while(isset($comments['comments'][$j])) {
				...
				$postcontentdiv .= "<span style='display:none';>" .$comments['comments'][$j]['commentid'] . ": </span><a href='?view=profile&username=" . $comments['comments'][$j]['username'] . "'>" . $comments['comments'][$j]['vorname'] . " " . $comments['comments'][$j]['nachname'] . "</a> kommentiere am " . $comments['comments'][$j]['date'] . ":<br />" . $comments['comments'][$j]['comment'];
